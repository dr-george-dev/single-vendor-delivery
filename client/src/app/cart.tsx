...
import { API_BASE_URL } from "../config/api"; // <-- Using your dynamic API config

// ✅ Pointing specifically to the orders endpoint for checkout
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderItems = items.map((item: any) => ({
        ...
        image: item.image, 
        price: item.price, 
        product: item._id || item.id // Fallback just in case
      }));

      // Sending POST request to the correct API_URL
      const response = await fetch(API_URL, {
        ...
        body: JSON.stringify({ 
          orderItems, 
          deliveryAddress: "172 Grand St, NY", 
          paymentMethod: "Card",
          subtotal,
          deliveryFee,
          totalPrice: total
      ...
      // ✅ Route dynamically to success with the newly generated Order ID
      router.push({
        pathname: "/success",
            ...
            {items.map((item: any) => (
              <View key={item._id || item.id} className="flex-row items-center mb-6">
                ...
                <View className="flex-row items-center bg-white rounded-full px-1 border border-gray-100 shadow-sm">
                  <TouchableOpacity onPress={() => item.qty > 1 ? addItem(item, -1) : removeItem(item._id || item.id)} className="w-8 h-8 rounded-full items-center justify-center">
      ...
      {items.length > 0 && (
        <View className="px-6 pb-8 pt-4">
          <TouchableOpacity 
            ...
            className={`h-16 rounded-full items-center justify-center flex-row shadow-md ${isCheckingOut ? 'bg-orange-300' : 'bg-[#1a1c1e]'}`}
          >
            {isCheckingOut ? (
              <ActivityIndicator color="white" />
            ) : (
              <Text className="text-white font-extrabold text-lg">Checkout • ${total.toFixed(2)}</Text>
            )}
          </TouchableOpacity>
        </View>
      )}
