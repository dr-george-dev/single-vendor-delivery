...
export default function CartScreen() {
  ...
  const { items, addItem, removeItem, getSubtotal, clearCart } = useCartStore(
    (state: any) => state,
  );
  ...
  const handlePlaceOrder = async () => {
    if (!user || !token) {
      ...
      router.push({
        pathname: "/success",
        params: { orderId: data._id }, // Assuming your backend returns the new order ID
      });
      return;
    }
            ...
            {items.map((item: any) => (
              ...
              >
                ...
                <View className="flex-row items-center bg-white rounded-full px-1 border border-gray-100 shadow-sm">
                  ...
                  <Text className="px-2 font-bold text-gray-900">
                    {item.qty}
                  </Text>
                  <TouchableOpacity
